				   ...
				   const u8 *data, const size_t len)
{
	...
	if ((chipset == 0x3070) ||
		...
		if (rt2800_efuse_detect(rt2x00dev))
			rt2800_mcu_request(rt2x00dev, MCU_CURRENT, 0xff, 0xff, 0);
		else
			rt2800_mcu_request(rt2x00dev, MCU_CURRENT, 0xff, 0xff, 0x4);
		udelay(10);
	}
...
static void rt2800usb_disable_radio(struct rt2x00_dev *rt2x00dev)
{
	...
	rt2800_register_write(rt2x00dev, USB_DMA_CFG, 0);
	...
	rt2800_register_write(rt2x00dev, PWR_PIN_CFG, 0x3);
				    ...
				    struct txentry_desc *txdesc)
{
	...
	/*
	 * TX frame format is :
	 * | TXINFO | TXWI | header | L2 pad | payload | pad | TXINFO | USBpad |
	 *          |<---------- tx_pkt_len ---------------->|
	 *
	 * L2 pad is used to have payload aligned on a 4 bytes boundary
	 *
	 * pad is used to have tx_pkt_len aligned on a 4 bytes boundary
	 *
	 * USB pad is 0 or 4 bytes. 4 bytes are needed only in the case where
	 * URB total size would be a multiple of the endpoint max packet size
	 * (512 bytes).
	 */
	...
	tx_pkt_len = TXWI_DESC_SIZE + skb->len;
	tx_pkt_len = roundup(tx_pkt_len, 4);

	rt2x00_set_field32(&word, TXINFO_W0_USB_DMA_TX_PKT_LEN, tx_pkt_len);
...
static int rt2800usb_get_tx_data_len(struct queue_entry *entry)
{
	 ...
	 */
	length = roundup(entry->skb->len + TXINFO_DESC_SIZE, 4);
				  ...
				  struct rxdone_entry_desc *rxdesc)
{
	...
	skb_trim(entry->skb, rx_pkt_len - RXWI_DESC_SIZE);
}
