				       ...
				       struct channel_info *info)
{
	...
	} else if (rt2x00dev->default_ant.tx == 2) {
				  ...
				  struct channel_info *info)
{
	...
	if ((rt2x00_rt(&rt2x00dev->chip, RT3070) ||
	     ...
	     rt2x00_rt(&rt2x00dev->chip, RT3572)) &&
	...
	else if (rt2x00_rf(&rt2x00dev->chip, RF2820) ||
			     ...
			     struct rt2x00lib_conf *libconf)
{
	...
	if (state == STATE_SLEEP) {
		if (rt2x00_rt(&rt2x00dev->chip, RT3070) ||
		   rt2x00_rt(&rt2x00dev->chip, RT3090)) {
			rt2800_rfcsr_read(rt2x00dev, 1, &rfcsr);
			rt2x00_set_field8(&rfcsr, RFCSR1_RFBLOCK_EN, 0);
			rt2800_rfcsr_write(rt2x00dev, 1, rfcsr);

			rt2800_rfcsr_read(rt2x00dev, 7, &rfcsr);
			rt2x00_set_field8(&rfcsr, RFCSR7_RF_VCO_IC, 0);
			rt2800_rfcsr_write(rt2x00dev, 7, rfcsr);

			rt2800_rfcsr_read(rt2x00dev, 9, &rfcsr);
			rt2x00_set_field8(&rfcsr, RFCSR9_R, 0);
			rt2800_rfcsr_write(rt2x00dev, 9, rfcsr);

			rt2800_rfcsr_read(rt2x00dev, 21, &rfcsr);
			rt2x00_set_field8(&rfcsr, RFCSR21_RX_CTB_EN, 0);
			rt2800_rfcsr_write(rt2x00dev, 21, rfcsr);

			if ((rt2x00_rev(&rt2x00dev->chip) == RT3070_VERSION) ||
			   rt2x00_rt(&rt2x00dev->chip, RT3090)) {
				rt2800_rfcsr_read(rt2x00dev, 27, &rfcsr);
				rt2x00_set_field8(&rfcsr, RFCSR27_TX_TO_RX_IQ, 3);
				rt2x00_set_field8(&rfcsr, RFCSR27_LDORF_VC, 1);
				rt2x00_set_field8(&rfcsr, RFCSR27_R, 7);
		...
		if ((rt2x00_rev(&rt2x00dev->chip) == RT3070_VERSION) ||
		   ...
		   rt2x00_rt(&rt2x00dev->chip, RT3572)) {
			...
			rt2x00_set_field32(&reg, LDO_CFG0_CORE_VLEVEL, 28);
		...
		if (rt2x00_rt(&rt2x00dev->chip, RT3070) ||
		   rt2x00_rt(&rt2x00dev->chip, RT3090)) {
			rt2800_rfcsr_read(rt2x00dev, 1, &rfcsr);
			rt2x00_set_field8(&rfcsr, RFCSR1_RFBLOCK_EN, 1);
			rt2800_rfcsr_write(rt2x00dev, 1, rfcsr);

			rt2800_rfcsr_read(rt2x00dev, 7, &rfcsr);
			rt2x00_set_field8(&rfcsr, RFCSR7_RF_VCO_IC, 3);
			rt2800_rfcsr_write(rt2x00dev, 7, rfcsr);

			rt2800_rfcsr_read(rt2x00dev, 9, &rfcsr);
			rt2x00_set_field8(&rfcsr, RFCSR9_R, 14);
			rt2800_rfcsr_write(rt2x00dev, 9, rfcsr);

			rt2800_rfcsr_read(rt2x00dev, 21, &rfcsr);
			rt2x00_set_field8(&rfcsr, RFCSR21_RX_CTB_EN, 1);
			rt2800_rfcsr_write(rt2x00dev, 21, rfcsr);

			if ((rt2x00_rev(&rt2x00dev->chip) == RT3070_VERSION) ||
			   rt2x00_rt(&rt2x00dev->chip, RT3090)) {
				if(rt2x00_rev(&rt2x00dev->chip) < RT3071E_VERSION) {
					rt2800_rfcsr_read(rt2x00dev, 27, &rfcsr);
					rt2x00_set_field8(&rfcsr, RFCSR27_TX_TO_RX_IQ, 3);
					rt2x00_set_field8(&rfcsr, RFCSR27_LDORF_VC, 0);
					rt2x00_set_field8(&rfcsr, RFCSR27_R, 0);
					rt2800_rfcsr_write(rt2x00dev, 27, rfcsr);
				} else {
					rt2800_rfcsr_read(rt2x00dev, 27, &rfcsr);
					rt2x00_set_field8(&rfcsr, RFCSR27_TX_TO_RX_IQ, 0);
					rt2x00_set_field8(&rfcsr, RFCSR27_LDORF_VC, 0);
					rt2x00_set_field8(&rfcsr, RFCSR27_R, 0);
					rt2800_rfcsr_write(rt2x00dev, 27, rfcsr);
				}
		...
		if ((rt2x00_rev(&rt2x00dev->chip) == RT3070_VERSION) ||
		   rt2x00_rt(&rt2x00dev->chip, RT3090) ||
		   rt2x00_rt(&rt2x00dev->chip, RT3572)) {
			if (test_bit(CONFIG_DAC_NEED_PATCH, &rt2x00dev->flags) &&
			    rt2x00_rev(&rt2x00dev->chip) < RT3071E_VERSION) {
				rt2800_register_read(rt2x00dev, LDO_CFG0, &reg);
				rt2x00_set_field32(&reg, LDO_CFG0_BGSEL, 1);
				rt2x00_set_field32(&reg, LDO_CFG0_CORE_VLEVEL, 12);
				rt2800_register_write(rt2x00dev, LDO_CFG0, reg);
			} else {
				rt2800_register_read(rt2x00dev, LDO_CFG0, &reg);
				rt2x00_set_field32(&reg, LDO_CFG0_BGSEL, 1);
				rt2x00_set_field32(&reg, LDO_CFG0_CORE_VLEVEL, 0);
				rt2800_register_write(rt2x00dev, LDO_CFG0, reg);
			}
		}
		if (rt2x00_rt(&rt2x00dev->chip, RT3572)) {
			rfcsr = 0;
			rt2x00_set_field8(&rfcsr, RFCSR8_R, 1);
			rt2800_rfcsr_write(rt2x00dev, 8, rfcsr);
		}
...
int rt2800_init_registers(struct rt2x00_dev *rt2x00dev)
{
	...
	if (rt2x00_intf_is_usb(rt2x00dev) &&
	    (rt2x00_rt(&rt2x00dev->chip, RT3070) ||
	     rt2x00_rt(&rt2x00dev->chip, RT3572)))
	...
	else if (rt2x00_rev(&rt2x00dev->chip) == RT3070_VERSION)
	...
	rt2x00_set_field32(&reg, MM40_PROT_CFG_PROTECT_CTRL, !rt2x00_intf_is_usb(rt2x00dev));
...
int rt2800_init_bbp(struct rt2x00_dev *rt2x00dev)
{
	...
	if (rt2x00_intf_is_usb(rt2x00dev) &&
		...
		if (rt2x00_rev(&rt2x00dev->chip) > RT3070_VERSION) {
			...
			rt2x00_set_field8(&value, BBP31_POWER, 0);
				...
				bool bw40, u8 rfcsr24, u8 filter_target)
{
	...
	rt2x00_set_field8(&rfcsr, RFCSR31_R0, bw40);
...
int rt2800_init_rfcsr(struct rt2x00_dev *rt2x00dev)
{
	...
	if (rt2x00_intf_is_usb(rt2x00dev) &&
		...
		rt2800_rfcsr_read(rt2x00dev, 6, &rfcsr);
		rt2x00_set_field8(&rfcsr, RFCSR6_R1, 1);
		rt2800_rfcsr_write(rt2x00dev, 6, rfcsr);
		...
		rt2x00_set_field32(&reg, LDO_CFG0_CORE_VLEVEL, 28);
		rt2800_register_write(rt2x00dev, LDO_CFG0, reg);
		//rt2800_register_write(rt2x00dev, LDO_CFG0, reg_tmp);
	} else if (rt2x00_intf_is_usb(rt2x00dev)) {
		rt2800_rfcsr_write(rt2x00dev, 4, 0x40);
		rt2800_rfcsr_write(rt2x00dev, 5, 0x03);
		rt2800_rfcsr_write(rt2x00dev, 6, 0x02);
	...
	if (rt2x00_intf_is_usb(rt2x00dev))
		rt2800_rfcsr_write(rt2x00dev, 27, 0x3);
...
int rt2800_validate_eeprom(struct rt2x00_dev *rt2x00dev)
{
	...
	if (word == 0x00ff) {
		...
		EEPROM(rt2x00dev, "NIC low: 0x%04x\n", word);
	}
	if (word == 0xff00) {
		rt2x00_set_field16(&word, EEPROM_NIC_BW40M_BG, 0);
		rt2x00_set_field16(&word, EEPROM_NIC_BW40M_A, 0);
		rt2x00_eeprom_write(rt2x00dev, EEPROM_NIC, word);
		EEPROM(rt2x00dev, "NIC high: 0x%04x\n", word);
	}
	...
	if ((word & 0xff00) == 0xff00) {
		...
		if (rt2x00_intf_is_usb(rt2x00dev))
			rt2x00_eeprom_write(rt2x00dev, EEPROM_LED3, 0x5627);
		else
			rt2x00_eeprom_write(rt2x00dev, EEPROM_LED3, 0xa9f8);
		EEPROM(rt2x00dev, "Freq led: 0x%04x\n", word);
	}
...
int rt2800_init_eeprom(struct rt2x00_dev *rt2x00dev)
{
	...
	if (rt2x00_intf_is_usb(rt2x00dev)) {
		...
		if (rt2x00_check_rev(chip, 0xfff00000, 0x28600000) ||
			...
			rt2x00_set_chip_rt(rt2x00dev, RT3572);
		} else {
	...
	/*
	 * Detect if this device need patch to fix DAC issue.
	 */
	if (rt2x00_get_field16(eeprom, EEPROM_NIC_DAC_NEED_PATCH))
		__set_bit(CONFIG_DAC_NEED_PATCH, &rt2x00dev->flags);
